...
| SwapExactInput | `amm_swap_a_to_b_private_unauthorized_destination_is_not_expressible` | EXIST, CHAIN | Swap paying out to a brand-new `PrivateForeignInit` destination (`npk` only, no `nsk`) | ❌ Not-expressible — on v0.2.4 the signer check is satisfied (PR #621 authorizes foreign-init), but AMM's own precondition still requires the swap destination to already be owned by the Token Program. **[Open — Programs]** |
...
| NewDefinition | `amm_new_definition_foreign_init_lp_holder` | EXIST | Pool creation minting the initial LP tokens to a `PrivateForeignInit` holder (`npk` only, no `nsk`) | ✅ — expressible on v0.2.4: PR #621 makes the fresh foreign-init `user_holding_lp` authorized, satisfying the guest's signer check. (Was not-expressible pre-#621.) |
...
| InitializeAccount | `token_initialize_private_account_via_foreign_init` | EXIST | `InitializeAccount`'s target is `#[account(init, signer)]`; a third party initializes it via `PrivateForeignInit` (no `nsk`, only the owner's `npk`) | ✅ — expressible on v0.2.4: PR #621 makes the fresh foreign-init pre-state `is_authorized == true`, satisfying the signer check. (Was not-expressible pre-#621.) |
...
| NewFungibleDefinition | `token_new_fungible_definition_foreign_init_holder` | EXIST | The initial holder is created via `PrivateForeignInit` (no `nsk`, only the holder's `npk`) — same self-service shape as `InitializeAccount`'s target | ✅ — expressible on v0.2.4 via PR #621, same as the `InitializeAccount` row above. (Was not-expressible pre-#621.) |


### Remarks
- On v0.2.4, init via `PrivateForeignInit` IS possible: PR #621 makes the fresh foreign-init pre-state `is_authorized = true`. The paired pre-state must therefore be built with `is_authorized: true`. **[Resolved — PR #621]**
- New token definition with a `PrivateForeignInit` Token holding is likewise permitted on v0.2.4 (the holding is authorized). **[Resolved — PR #621]**

# Conclusions

> **v0.2.4 note.** These tests are pinned against `logos-execution-zone` **v0.2.4**, which already
> includes PR #621 and PR #625. Two consequences: (1) `PrivateUnauthorized` was renamed to
> `PrivateForeignInit` and its fresh pre-state is now `is_authorized == true`, so the init/new-def
> cases the report first recorded as not-expressible are now covered by *passing positive* tests
> (`token_initialize_private_account_via_foreign_init`,
> `token_new_fungible_definition_foreign_init_holder`, `amm_new_definition_foreign_init_lp_holder`).
> (2) The `amm_swap...unauthorized_destination` case is still not-expressible, but the blocker moved
...
| Owner | Item | Status |
|---|---|---|
| Zones | `is_authorized = false` blocks `PrivateUnauthorized` on signer-gated instructions | ✅ Resolved — [PR #621](https://github.com/logos-blockchain/logos-execution-zone/pull/621), in v0.2.4 (covered by positive tests) |
| Zones | Public transactions don't detect a silently-dropped declared account | ✅ Resolved — [PR #625](https://github.com/logos-blockchain/logos-execution-zone/pull/625), in v0.2.4 |
...
The resolved Zones items above are included in the `logos-execution-zone` **v0.2.4** tag that this
crate pins, so the corresponding positive tests exercise them directly.