...
void WalletIdlRegistry::registerProgram(const QString& programId,
                                        const QString& programName,
                                        const QByteArray& idlJson)
{
    if (!programId.isEmpty() && !programName.isEmpty() && !idlJson.isEmpty())
        m_programs.insert(programId, { programName, idlJson });
}

QVector<WalletDecodedProgram> WalletIdlRegistry::decode(
    const QVector<WalletAccountRead>& accounts) const
{
    QHash<QString, QVector<WalletAccountRead>> grouped;
    for (const WalletAccountRead& account : accounts) {
        if (m_programs.contains(account.programOwner))
            grouped[account.programOwner].append(account);
    }

    QVector<WalletDecodedProgram> decoded;
    decoded.reserve(grouped.size());
    for (auto iterator = grouped.cbegin(); iterator != grouped.cend(); ++iterator) {
        const Program program = m_programs.value(iterator.key());
        decoded.append({
            iterator.key(),
            program.name,
            WalletIdlDecoder::decode(program.idl, iterator.value()),
        });
    }
    return decoded;
}
